...
func EpayNotify(c *gin.Context) {
	...
	if verifyInfo.TradeStatus == epay.StatusTradeSuccess {
		...
		topUp, quotaToAdd, rebateResult, err := model.CompleteEpayRecharge(verifyInfo.ServiceTradeNo, verifyInfo.Type)
		...
		if err != nil {
			logger.LogError(c.Request.Context(), fmt.Sprintf("易支付 更新充值订单失败 trade_no=%s client_ip=%s error=%q verify_info=%q", verifyInfo.ServiceTradeNo, c.ClientIP(), err.Error(), common.GetJsonString(verifyInfo)))
			_, _ = c.Writer.Write([]byte("fail"))
			return
		}
		...
		// 重入补发的返利（quotaToAdd == 0 的重复回调）也需要记录日志
		model.RecordAffiliateRechargeRebateLog(rebateResult)
	} else {
